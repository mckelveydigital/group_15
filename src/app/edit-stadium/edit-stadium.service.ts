...
export class EditStadiumService {
  ...
  private stadiumUpdateUrl: string;
  public theresult;
  result: Object;
  stadium_update;
  ...
  editStadiumApi(theId, body) : Observable<any>{

    let headers = new Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
    let options = new RequestOptions({ headers: headers });

    this.stadiumUpdateUrl = this.teamApiUrl + "/" + theId + "/stadium/";

    this.stadium_update = this.http.post(this.stadiumUpdateUrl, body)
     .map(this.extractData)
     .catch(this.handleError);

     return this.stadium_update;
